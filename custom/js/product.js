...
// Pega esto en tu archivo JS, reemplazando la función editProduct existente.

function editProduct(productId = null) {

	if (productId) {
		// --- CORRECCIÓN 1: Limpiar ambos IDs ---
		$("#productId").remove();
		$("#editProductImageId").remove();
		...
		$.ajax({
			...
			success: function (response) {
				...
				var productImage = response.product_image ? response.product_image : '../assests/images/stock/default.png';
				$("#getProductImage").attr('src', productImage);
				...
				$("#editProductImage").fileinput({
					...
					elErrorContainer: '#kv-avatar-errors-1',
					msgErrorClass: 'alert alert-block alert-danger',
					defaultPreviewContent: '<img src="../assests/images/stock/default.png" alt="Imagen del Producto" style="width:100%;">',
					layoutTemplates: { main2: '{preview} {remove} {browse}' },
					allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"],
					initialPreview: [productImage],
					initialPreviewAsData: true,
					initialPreviewConfig: [
						{ caption: "Imagen actual", size: 0, url: productImage, key: 1 }
					]
				});

				// --- CORRECCIÓN 2: IDs únicos para cada formulario ---
				// ID para el formulario de texto
				$(".editProductFooter").append('<input type="hidden" name="productId" id="productId" value="' + response.product_id + '" />');
				// ID para el formulario de imagen (con el name/id que PHP espera)
				$(".editProductPhotoFooter").append('<input type="hidden" name="editProductImageId" id="editProductImageId" value="' + response.product_id + '" />');
				...
				$("#updateProductImageForm").unbind('submit').bind('submit', function (e) {
					e.preventDefault();
					...
					// --- CORRECCIÓN 3: Leer el ID del campo oculto correcto ---
					var productId = $("#editProductImageId").val();

					// console.log("ID del producto:", productId); // Debug
					// console.log("Imagen seleccionada:", productImage); // Debug

					if (productImage == "") {
						$("#editProductImage").closest('.center-block').after('<p class="text-danger">Se requiere seleccionar una imagen</p>');
						$('#editProductImage').closest('.form-group').addClass('has-error');
						return false;
					} else {
						// remove error text field
						$("#editProductImage").closest('.form-group').find('.text-danger').remove();
						// success out for form 
					...
					if (productImage) {
						// submit loading button
						$("#editProductImageBtn").prop('disabled', true).html('<i class="glyphicon glyphicon-refresh glyphicon-refresh-animate"></i> Guardando...');
						...
						var ajaxTimeout = setTimeout(function () {
							$("#editProductImageBtn").prop('disabled', false).html('<i class="glyphicon glyphicon-ok-sign"></i> Guardar Cambios');
							$('#edit-productPhoto-messages').html('<div class="alert alert-danger">' +
								'<button type="button" class="close" data-dismiss="alert">&times;</button>' +
								'<strong><i class="glyphicon glyphicon-warning-sign"></i></strong> La operación excedió el tiempo límite. Por favor, intente nuevamente.' +
								'</div>');
						}, 30000);
						...
						$.ajax({
							...
							success: function (response) {
								...
								if (response.success == true) {
									// console.log("Imagen actualizada con éxito"); // Debug
									...
									// --- MANEJO DE ERRORES DEL PHP ---
									$('#edit-productPhoto-messages').html('<div class="alert alert-danger">' +
										'<button type="button" class="close" data-dismiss="alert">&times;</button>' +
										'<strong><i class="glyphicon glyphicon-warning-sign"></i></strong> ' + (response.messages ? response.messages.join(', ') : 'Error desconocido.') +
										'</div>');
								}
...
} // /edit product function// /edit product function
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.success == true) {
						...
						// --- CORRECCIÓN 1: Ocultar modal también si hay error ---
						$("#removeProductModal").modal('hide');

						// --- CORRECCIÓN 2: Usar 'alert-danger' (rojo) para errores ---
						// Y usar el selector correcto '.remove-messages'
						$(".remove-messages").html('<div class="alert alert-danger">' +
							'<button type="button" class="close" data-dismiss="alert">&times;</button>' +
							'<strong><i class="glyphicon glyphicon-exclamation-sign"></i></strong> ' + response.messages +
							'</div>');

						// --- CORRECCIÓN 3: Apuntar a 'alert-danger' para ocultarlo ---
				...
				// --- CORRECCIÓN 4: Añadir 'error' handler para fallos de red/servidor ---
				error: function (xhr, status, error) {
					// Resetear el botón si el servidor da error 500 o 404
					$("#removeProductBtn").button('reset');
					$("#removeProductModal").modal('hide');

					$(".remove-messages").html('<div class="alert alert-danger">' +
						'<button type="button" class="close" data-dismiss="alert">&times;</button>' +
						'<strong><i class="glyphicon glyphicon-exclamation-sign"></i></strong> Error de servidor, intente de nuevo. ' + error +
						'</div>');
